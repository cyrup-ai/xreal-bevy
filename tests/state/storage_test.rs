...
#[tokio::test]async fn test_backup_operations() {
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let config = StorageConfig {
        base_directory: temp_dir.path().to_path_buf(),
...
#[tokio::test]
async fn test_atomic_writes() {    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let config = StorageConfig {
        base_directory: temp_dir.path().to_path_buf(),
        atomic_writes: true,
        ..Default::default()
...
#[tokio::test]
async fn test_storage_statistics() {    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let config = StorageConfig {
        base_directory: temp_dir.path().to_path_buf(),
        backup_config: BackupConfig {
            enabled: true,
