...
impl AtomicPluginState {
    ...
    pub fn is_loaded(&self) -> bool {
        matches!(self.get_lifecycle_state(), Self::STATE_LOADED | Self::STATE_RUNNING)
    }
}
...
pub fn create_plugin_description(s: &str) -> PluginDescription {
    PluginDescription::from_str(s).unwrap_or_else(|_| {
        ...
        if actual_len > 0 {
            PluginDescription::from_str(&s[..actual_len]).unwrap_or_else(|_| PluginDescription::new())
        } else {
...
impl PluginResourceLimits {
    ...
    pub const fn with_memory_limit(self, limit_mb: u32) -> Self {
        Self { max_memory_mb: limit_mb, ..self }
    }
    
    #[inline(always)]
    pub const fn with_texture_limit(self, size: u32) -> Self {
        Self { max_texture_size: size, ..self }
    }
}
...
impl PluginRenderStats {
    ...
    pub fn record_frame(&self, render_time_us: u32) {
        ...
        self.total_render_time_us.fetch_add(render_time_us as usize, Ordering::Relaxed);
    ...
    pub fn record_draw_call(&self, vertex_count: u32, triangle_count: u32) {
        ...
        self.vertices_processed.fetch_add(vertex_count as usize, Ordering::Relaxed);
        self.triangles_rendered.fetch_add(triangle_count as usize, Ordering::Relaxed);
    }
...
impl PluginSystemMetrics {
    ...
    pub fn record_plugin_load(&self, plugin_count: usize, load_time_us: u32) {
        ...
        self.load_time_us.fetch_add(load_time_us as usize, Ordering::Relaxed);
    }
    
    #[inline]
    pub fn record_plugin_unload(&self, plugin_count: usize, unload_time_us: u32) {
        self.active_plugins.store(plugin_count, Ordering::Relaxed);
        self.unload_time_us.fetch_add(unload_time_us as usize, Ordering::Relaxed);
    }
    
    #[inline]
    pub fn record_memory_usage(&self, cpu_bytes: usize, gpu_bytes: usize) {
        self.total_memory_usage_bytes.store(cpu_bytes, Ordering::Relaxed);
        self.total_gpu_memory_bytes.store(gpu_bytes, Ordering::Relaxed);
    }
    
    #[inline]
    pub fn record_event_processing(&self, processed: usize, dropped: usize) {
        self.events_processed.fetch_add(processed, Ordering::Relaxed);
    ...
    pub fn allocate(&self, size: usize) -> Option<*mut u8> {
        ...
        for i in 0..POOL_SIZE {
            ...
            if (current_bitmap & block_mask) == 0 {
                ...
                let old_bitmap = self.allocation_bitmap.fetch_or(block_mask, Ordering::AcqRel);
                if (old_bitmap & block_mask) == 0 {
                    // Successfully allocated
                    self.blocks[i].allocated.store(size as u64, Ordering::Release);
    ...
    pub fn deallocate(&self, ptr: *mut u8) -> bool {
        ...
        for i in 0..POOL_SIZE {
            ...
            if ptr_addr >= block_start && ptr_addr < block_end {
                let block_mask = 1u64 << i;
                let old_bitmap = self.allocation_bitmap.fetch_and(!block_mask, Ordering::AcqRel);
                if (old_bitmap & block_mask) != 0 {
                    // Successfully deallocated
                    let size = self.blocks[i].allocated.swap(0, Ordering::AcqRel);
                    self.total_allocated.fetch_sub(size as usize, Ordering::Relaxed);
                    return true;
                }
            }
        }
