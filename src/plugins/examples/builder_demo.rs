...
pub fn create_terminal_plugin() -> PluginMetadata {
    PluginBuilder::new()
        ...
        .version("1.3.2")
        .description("Professional terminal emulator with syntax highlighting and shell integration")
...
impl crate::plugins::PluginApp for ExamplePlugin {
    fn id(&self) -> &str { "com.example.demo" }
    ...
    fn initialize(&mut self, _context: &crate::plugins::PluginContext) -> anyhow::Result<()> { Ok(()) }
    fn render(&mut self, _context: &mut crate::plugins::RenderContext) -> anyhow::Result<()> { Ok(()) }
    fn handle_input(&mut self, _event: &crate::plugins::InputEvent) -> anyhow::Result<bool> { Ok(false) }
    fn update(&mut self, _delta_time: f32) -> anyhow::Result<()> { Ok(()) }
    fn resize(&mut self, _new_size: (u32, u32)) -> anyhow::Result<()> { Ok(()) }
