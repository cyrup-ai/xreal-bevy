...
use super::{
    fast_data::{
        AtomicPluginState, LockFreeRingBuffer, PluginAuthor, PluginDependencies, PluginDescription,
        PluginId, PluginName, PluginVersion, SmallString,
    },
    PluginApp, PluginCapabilitiesFlags, PluginMetadata, PluginSystemConfig,
};
...
impl PluginEntry {
    ...
    fn record_frame_time(&self, time_us: u32) {
        ...
        self.total_render_time_us
            .fetch_add(time_us as usize, Ordering::Relaxed);
    }
...
impl FastPluginRegistry {
    ...
    pub fn register_plugin(
        ...
        // Update lookup table
        self.lookup_table
            .insert(metadata.id.as_str().to_string(), plugin_index);
        ...
        info!(
            "🚀 Plugin '{}' registered at index {} (ultra-fast path)",
    ...
    pub fn unregister_plugin(&mut self, plugin_id: &str) -> Result<()> {
        ...
        info!(
            "🔥 Plugin '{}' unregistered (ran for {}ms)",
            plugin_id, run_time_ms
        );
...
pub fn fast_plugin_event_system(
    ...
    for event in events {
        match event {
            ...
            FastPluginEvent::PluginUnloaded {
                plugin_id,
                run_time_ms,
            } => {
                ...
                info!(
                    "Plugin {} unloaded after {}ms",
                    plugin_id.as_str(),
                    run_time_ms
                );
            }
            FastPluginEvent::PerformanceViolation {
                plugin_id,
                avg_frame_time_ms,
                threshold_ms,
            } => {
                warn!(
                    "Plugin {} performance violation: {:.2}ms > {:.2}ms threshold",
                    plugin_id.as_str(),
                    avg_frame_time_ms,
                    threshold_ms
                );
