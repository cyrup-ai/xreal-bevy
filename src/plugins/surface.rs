...
impl SurfaceManager {
    ...
    pub fn create_surface(&mut self, plugin_id: String, size: (u32, u32)) -> Result<String> {
        if self.plugin_surfaces.len() >= self.max_surfaces {
            return Err(PluginError::SurfaceError("Maximum surface limit reached".to_string()).into());
        }
        ...
        info!("Creating surface for plugin: {} ({}x{})", plugin_id, size.0, size.1);
    ...
    pub fn update_surface_transform(&mut self, plugin_id: &str, position: Vec3, visible: bool) -> Result<()> {
    ...
    pub fn resize_surface(&mut self, plugin_id: &str, new_size: (u32, u32)) -> Result<()> {
        if let Some(surface) = self.plugin_surfaces.get_mut(plugin_id) {
            ...
            info!("Resized surface for plugin: {} to {}x{}", plugin_id, new_size.0, new_size.1);
        }
        Ok(())
    }
    ...
    pub fn get_visible_surfaces(&self) -> Vec<&PluginSurface> {
        let mut surfaces: Vec<&PluginSurface> = self.plugin_surfaces
            .values()
    ...
    pub fn get_total_memory_usage(&self) -> u64 {
        self.plugin_surfaces
            ...
            .map(|surface| {
                ...
                let bytes_per_pixel = surface.config.format.block_copy_size(Some(wgpu::TextureAspect::All)).unwrap_or(4) as u64;
