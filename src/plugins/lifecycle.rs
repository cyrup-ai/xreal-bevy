...
impl PluginLifecycleManager {
    ...
    pub fn request_transition(&mut self, plugin_id: String, to_state: PluginLifecycleState) -> Result<()> {
        let current_state = self.plugin_states.get(&plugin_id).copied().unwrap_or(PluginLifecycleState::Loading);
        
        // Validate transition is allowed
        if !self.is_valid_transition(current_state, to_state) {
            return Err(anyhow::anyhow!("Invalid state transition for plugin {}: {:?} -> {:?}", 
                                     plugin_id, current_state, to_state));
        }
    ...
    pub fn get_plugin_state(&self, plugin_id: &str) -> PluginLifecycleState {
        self.plugin_states.get(plugin_id).copied().unwrap_or(PluginLifecycleState::Loading)
    }
...
pub fn plugin_lifecycle_system(
    ...
    for transition in transitions {
        ...
        lifecycle_manager.plugin_states.insert(transition.plugin_id.clone(), transition.to_state);
        ...
        match transition.to_state {
            ...
            PluginLifecycleState::Error => {
                let error_msg = lifecycle_manager.error_states
        ...
        debug!("Plugin {} transitioned from {:?} to {:?}", 
...
pub fn plugin_health_monitoring_system(
    ...
    for plugin_id in running_plugins {
        ...
        if !performance_tracker.is_plugin_performing_well(&plugin_id) {
            ...
            if avg_frame_time > 32.0 { // More than 2x 60fps budget
                warn!("Pausing plugin {} due to severe performance impact", plugin_id);
                let _ = lifecycle_manager.request_transition(plugin_id.clone(), PluginLifecycleState::Paused);
            }
        }
        ...
        if plugin_registry.get_plugin(&plugin_id).is_none() {
            ...
            lifecycle_manager.record_error(plugin_id, "Plugin instance missing from registry".to_string());
        }
    }
}
    ...
    for plugin_id in error_plugins {
        if let Some(instance) = plugin_registry.get_plugin(&plugin_id) {
            ...
            if time_since_error > 5 {
                ...
                match lifecycle_manager.request_transition(plugin_id.clone(), PluginLifecycleState::Loading) {
                    Ok(_) => {
...
pub fn plugin_resource_coordination_system(
    ...
    for plugin_id in initializing_plugins {
        ...
        if resource_manager.get_memory_usage() < resource_manager.resource_limits.max_total_memory_mb {
            if let Err(e) = resource_manager.register_plugin(16) { // 16MB base allocation
                warn!("Failed to allocate base memory for plugin {}: {}", plugin_id, e);
            }
        }
    }
