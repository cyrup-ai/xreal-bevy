...
impl PluginLoader {
    ...
    pub fn load_plugin(&mut self, library_path: &Path) -> Result<(Box<dyn PluginApp>, PluginMetadata)> {
        ...
        self.security_validator.validate_capabilities(&metadata.capabilities)?;
    ...
    pub fn validate_manifest(&self, manifest_path: &Path) -> Result<PluginManifest> {
        ...
        if manifest.plugin.version.is_empty() {
            return Err(PluginError::LoadFailed("Plugin version cannot be empty".to_string()).into());
        }
    ...
    fn get_or_load_library(&mut self, path: &Path) -> Result<Arc<Library>> {
        ...
        self.library_cache.insert(path.to_path_buf(), library_arc.clone());
    ...
    fn extract_metadata(&self, library: &Library) -> Result<PluginMetadata> {
        let get_metadata: Symbol<extern "C" fn() -> PluginMetadata> = unsafe {
            library.get(b"get_plugin_metadata").map_err(|e| {
                PluginError::LoadFailed(format!("Plugin missing get_plugin_metadata function: {}", e))
            })?
        };
    ...
    fn is_version_compatible(&self, required: &str, current: &str) -> bool {
        ...
        let required_parts: Vec<u32> = required.split('.')
...
impl SecurityValidator {
    ...
    fn validate_library(&self, library_path: &Path) -> Result<()> {
        ...
        if !valid_extensions.contains(&extension) {
            return Err(PluginError::LoadFailed(
                format!("Unsupported library extension: {}", extension)
            ).into());
        }
        
        // Check if library is in allowed directory
        let is_in_allowed_dir = self.config.plugin_directories.iter().any(|dir| {
            library_path.starts_with(dir)
        });
...
"#, plugin_id, plugin_name, plugin_name)
    ...
    pub fn validate_dev_setup(plugin_dir: &Path) -> Result<()> {
        ...
        if !cargo_content.contains(r#"crate-type = ["cdylib"]"#) {
            return Err(anyhow::anyhow!("Cargo.toml must specify crate-type = [\"cdylib\"]"));
        }
