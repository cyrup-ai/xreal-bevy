...
use super::{
    context::OrientationAccess, PluginApp, PluginError, PluginInstance, PluginMetadata,
    PluginState, PluginSystemConfig,
};
use crate::tracking::{CalibrationState, Orientation};
...
impl PluginRegistry {
    ...
    pub fn load_plugin(&mut self, plugin_id: &str) -> Result<()> {
        ...
        let create_plugin: Symbol<extern "C" fn() -> Box<dyn PluginApp>> =
        ...
        self.orientation_channels.insert(
            plugin_id.to_string(),
            (orientation_tx, orientation_rx.clone()),
        );
        self.calibration_channels.insert(
            plugin_id.to_string(),
            (calibration_tx, calibration_rx.clone()),
        );
        ...
        self.libraries
            .insert(plugin_id.to_string(), Arc::new(library));
    ...
    fn extract_metadata(&self, library_path: &Path) -> Result<PluginMetadata> {
        ...
        let get_metadata: Symbol<extern "C" fn() -> PluginMetadata> =
            unsafe { library.get(b"get_plugin_metadata")? };
    ...
    fn setup_hot_reload(&mut self) -> Result<()> {
        ...
        info!(
            "Hot reload watcher enabled for {} directories",
            self.config.plugin_directories.len()
        );
        Ok(())
    }
}
