...
impl Plugin for InputSystemPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.configure_sets(
            Update,
            InputSystemSet
                .after(bevy::transform::TransformSystem::TransformPropagate),  // After transforms are updated
        );
...
pub fn handle_input(
    ...
    if let Ok((transform, screen)) = query_plane.single() {
        ...
        if let Some(intersection) = ray.intersect_plane(transform.translation(), plane_normal) {
            ...
            let local_pos = transform.compute_matrix().inverse().transform_point3(intersection);
