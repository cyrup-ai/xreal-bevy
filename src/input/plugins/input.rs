...
impl InputSystem {
    ...
    fn map_key_code(&self, key: KeyCode) -> Result<enigo::Key, InputError> {
        ...
        match key {
            ...
            K::KeyY | K::KeyZ => 
                Err(InputError::UnsupportedInput("Alphabet keys are not supported in enigo v0.5.0 on macOS".to_string())),
            ...
            K::Digit8 | K::Digit9 => 
                Err(InputError::UnsupportedInput("Number keys are not supported in enigo v0.5.0 on macOS".to_string())),
            ...
            K::NumpadAdd | K::NumpadSubtract | K::NumpadMultiply | K::NumpadDivide |
            K::NumpadDecimal => 
                Err(InputError::UnsupportedInput("Numpad keys are not supported in enigo v0.5.0 on macOS".to_string())),
            
            // Catch-all for other unsupported keys
            _ => Err(InputError::UnsupportedInput(format!("Key not supported in enigo v0.5.0 on macOS: {:?}", key))),
        }
    }
}
...
fn handle_mouse_input(
    ...
    for event in mouse_motion.read() {
        if let Err(e) = input_system.move_mouse(
