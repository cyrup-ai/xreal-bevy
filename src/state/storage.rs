...
use crate::state::{schema::core::PersistentAppState, StateError, StateSerializer};
...
impl StateStorage {
    ...
    pub fn with_config(config: StorageConfig) -> Result<Self> {
        ...
        if storage.config.backup_config.enabled {
            let backup_dir = storage
                .config
                .base_directory
                .join(&storage.config.backup_config.backup_directory);
            std::fs::create_dir_all(&backup_dir).map_err(StateError::StorageError)?;
        }
    ...
    pub async fn save_state(&self, persistent_state: &PersistentAppState) -> Result<()> {
        ...
        if serialized_data.len() as u64 > self.config.max_file_size {
            return Err(StateError::StorageError(std::io::Error::new(
                ...
                format!(
                    "State file too large: {} bytes > {} bytes limit",
                    serialized_data.len(),
    ...
    pub async fn list_backups(&self) -> Result<Vec<BackupInfo>> {
        ...
        let backup_dir = self
            .config
            .base_directory
            .join(&self.config.backup_config.backup_directory);
            ...
            .map_err(StateError::StorageError)?
        {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                let metadata = entry.metadata().await.map_err(StateError::StorageError)?;

                let modified = metadata.modified().map_err(StateError::StorageError)?;
    ...
    pub async fn restore_from_backup(&self, backup_path: &Path) -> Result<()> {
        ...
        fs::copy(backup_path, &primary_path)
            .await
            .map_err(StateError::StorageError)?;
    ...
    async fn atomic_write(&self, file_path: &Path, data: &str) -> Result<()> {
        ...
        // Write to temporary file
        {
            ...
            temp_file.flush().await.map_err(StateError::StorageError)?;
    ...
    async fn direct_write(&self, file_path: &Path, data: &str) -> Result<()> {
        ...
        file.flush().await.map_err(StateError::StorageError)?;

        // Ensure data is written to disk
        file.sync_all().await.map_err(StateError::StorageError)?;
...
impl AsyncStateStorage {
    ...
    pub fn load_state_async(
        &self,
    ) -> impl std::future::Future<Output = Result<PersistentAppState>> {
        let storage = self.storage.clone();

        async move { storage.load_state().await }
    }
    ...
    pub fn spawn_load_task(&self) -> bevy::tasks::Task<Result<PersistentAppState>> {
        ...
        self.task_pool
            .spawn(async move { storage.load_state().await })
    }
    ...
    pub fn spawn_cleanup_task(&self) -> bevy::tasks::Task<Result<()>> {
        ...
        self.task_pool
            .spawn(async move { storage.cleanup_old_backups().await })
    }
...
pub mod utils {
    ...
    pub fn calculate_directory_size(
        ...
        Box::pin(async move { calculate_directory_size_impl(path).await })
    }

    async fn calculate_directory_size_impl(path: &Path) -> Result<u64> {
        let mut total_size = 0;
        let mut entries = fs::read_dir(path).await.map_err(StateError::StorageError)?;
